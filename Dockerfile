...
# Path crate needed for manifest resolution (root package depends on aardvark-sys).
COPY crates/aardvark-sys crates/aardvark-sys
# Keep only the root crate in the workspace (path crates are excluded by .dockerignore).
